...
import time
import logging
logging.getLogger("scapy.runtime").setLevel(logging.INFO) # Disable the annoying No Route found warning !
...
if __name__ == '__main__':
    ...
    if is_up(ip):
        print(f"Host {ip} is up, start scanning")
        for ports in chunks(range(1, 1024), 100):
            results = is_open(ip, ports)
            for p, r in results.items():
                print(f"{p} : {r}")
        duration = time.time() - start_time
        print(f"{ip} Scan Completed in {duration}s")
    else:
